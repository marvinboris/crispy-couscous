...
export default function LanguageBlock({ language, selected, select }: LanguageBlockProps) {
    return <div onClick={() => select(language.abbr)} className={classNames("rounded-[25px] w-[106px] flex flex-col cursor-pointer items-center p-[13px]", selected ? 'bg-white border relative border-green-600 shadow-lg shadow-primary-600/20' : 'bg-secondary-100')}>
        {selected && <div className="absolute top-2 right-2 translate-x-1/2 -translate-y-1/2 after:absolute after:inset-2 after:bg-white after:-z-10">
            <CheckCircleIcon className="w-9 text-green-600 z-10" />
        </div>}
        ...
        <div className={classNames(selected ? 'text-white bg-green-600' : 'text-secondary-700 bg-primary-600/20', "w-[63px] text-center py-[5.5px] font-medium text-xs rounded-3xl")}>{language.name}</div>
    </div>
}